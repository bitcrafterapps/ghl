            ...
            <div className="flex items-center gap-4">
              {selectedJob && (
                <Link
                  ...
                  className="p-2 text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/10 rounded-lg transition-colors mr-2"
                  title="Back to Job"
                >
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Job Photos</h1>
                <p className="text-sm text-gray-500 dark:text-gray-400">Manage before/after photos and publish to gallery</p>
              </div>
            </div>
        ...
        <div className="mb-8">
          <div className="relative">
            <button
              ...
              className="w-full max-w-md flex items-center justify-between px-4 py-3 bg-white dark:bg-[#1C1C1C] border border-gray-200 dark:border-white/10 rounded-xl text-left hover:border-gray-300 dark:hover:border-white/20 transition-colors"
            >
              {selectedJob ? (
                <div>
                  <p className="text-sm text-gray-500">
                    <span className="text-gray-500">{selectedJob.jobNumber}</span>
                  </p>
                  <p className="text-gray-900 dark:text-white font-medium">{selectedJob.title}</p>
                </div>
              ) : (
            ...
            {showJobSelector && (
              <>
                ...
                <div className="absolute top-full left-0 mt-2 w-full max-w-md max-h-80 overflow-y-auto bg-white dark:bg-[#1C1C1C] border border-gray-200 dark:border-white/10 rounded-xl shadow-xl z-20">
                  <button
                    onClick={() => {
                      setSelectedJob(null);
                      setShowJobSelector(false);
                    ...
                    className="w-full px-4 py-3 text-left hover:bg-gray-50 dark:hover:bg-white/5 transition-colors border-b border-gray-200 dark:border-white/5 text-gray-500 dark:text-gray-400 font-medium sticky top-0 bg-white dark:bg-[#1C1C1C] z-10"
                  >
                  ...
                  {jobs.length === 0 ? (
                    ...
                    jobs.map(job => (
                      <button
                        ...
                        className={cn(
                          "w-full px-4 py-3 text-left hover:bg-gray-50 dark:hover:bg-white/5 transition-colors border-b border-gray-200 dark:border-white/5 last:border-0",
                          selectedJob?.id === job.id && "bg-pink-50 dark:bg-pink-500/10"
                        )}
                      >
            ...
            <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2">
              <button
                ...
                className={cn(
                  ...
                  filterType === 'all'
                    ? "bg-pink-500/10 dark:bg-pink-500/20 border-pink-500/30 dark:border-pink-500/50 text-pink-600 dark:text-pink-400"
                    : "bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white"
                )}
              >
                All ({photos.length})
              </button>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md p-6 bg-white dark:bg-[#1C1C1C] border border-gray-200 dark:border-white/10 rounded-2xl shadow-2xl">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Delete Photo</h3>
