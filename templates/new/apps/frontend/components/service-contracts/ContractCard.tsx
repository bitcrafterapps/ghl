    ...
    <div className="group relative bg-[#1C1C1C]/80 backdrop-blur-sm rounded-xl border border-white/5 p-5 transition-all duration-300 hover:border-white/10 hover:bg-[#1C1C1C] hover:shadow-lg hover:shadow-black/20">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button
              ...
              className="p-1.5 rounded-lg text-gray-400 hover:text-white hover:bg-white/10 transition-colors opacity-0 group-hover:opacity-100"
            >
      ...
      <div className="flex items-center justify-between pt-3 border-t border-white/5 text-xs">
        <div className="text-gray-500">
          Start: <span className="text-gray-400">{formatDate(contract.startDate)}</span>
        </div>
        {contract.endDate && (
          <div className={cn(
            "text-gray-500",
            isExpiringSoon && contract.status === 'active' && "text-orange-400"
          )}>
            Ends: <span className={isExpiringSoon && contract.status === 'active' ? "text-orange-400" : "text-gray-400"}>
      ...
      {contract.servicesIncluded && contract.servicesIncluded.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-3 pt-3 border-t border-white/5">
          ...
          {contract.servicesIncluded.length > 3 && (
            <span className="px-2 py-0.5 text-[10px] rounded-full bg-white/5 text-gray-400">
              +{contract.servicesIncluded.length - 3}
            </span>
          )}
        </div>
