...
export default function GalleryManagementPage() {
    ...
    if (token) {
      ...
      fetch(`${apiUrl}/api/v1/users/profile`, {
        ...
        .then(data => {
          ...
          return fetch(`${apiUrl}/api/v1/gallery-images`, {
            headers: { 'Authorization': `Bearer ${token}` }
          });
        })
        .then(res => {
  ...
  const handleUpload = async (file: File, metadata: Partial<GalleryImage>) => {
    ...
    // Include company ID from siteConfig to associate the image with this company
    if ((siteConfig as any).companyId) formData.append('companyId', String((siteConfig as any).companyId));
